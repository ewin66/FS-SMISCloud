...
function getVibrationList() {
    $('#show_Char').children().remove();
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length == 0) {
                $('#vibrationList').selectpicker('refresh');
                return;
            }
            var option = '';
            //索力下面有两种sensortype
            ...
            if (defaultSensorID == '' || defaultSensorID == null || defaultSensorID == undefined) {
                ...
                $('#vibrationList').selectpicker('val', defaultSensorID);
            }
            else {
                $('#vibrationList').selectpicker('val', parseInt(defaultSensorID));
            }
            ...
            getCollectTime();
            $('#vibrationList').unbind('change').change(function () {
                $('#vibrationList').selectpicker('refresh');
                getCollectTime();
...
function v_Query() {
    //查询指定时间段下以计算的微震数据
    var index = $('#collectTime').next().find(".selected").attr("rel");
    $("#show_Char").empty();
    if ($('#collectTime').find("option").length == 0) {
        alertTips('该传感器没有采集信息!', 'label-important', 'tip', 5000);
        return;
    }
    var time = $('#collectTime').find("option")[index].value;

    var collectTime = ChangeDateFormat(time);
    var factorId = document.getElementById('HiddenFactorNo').value;
    var url = apiurl + '/sensors/' + structid + '/info/' + factorId + '/list?token=' + getCookie('token');
            ...
            if (data.length != 0) {
                ...
                $(data).each(function (i, item) {
                    if (item.items.length != 0) {
                        if (item.items[2].value < minZ) {
                            minZ = item.items[2].value;
                        }
                        if (item.items[2].value > maxZ) {
                            maxZ = item.items[2].value;
                        }
                        var obj = new Object();
                        ...
                        obj.data = [[item.items[0].value, item.items[1].value, item.items[2].value]];
                        coords[i] = obj;
                    }
                });
                var origin = new Object();
                var url = apiurl + '/vibration/' + structid + '/' + collectTime + '/microsetsmic?token=' + getCookie('token');
                $.ajax({
                    url: url,
                    type: 'get',
                    cache: false,
                    success: function (data) {
                        if (data.length != 0) {
                            origin.name = "震源";
                            origin.color = "red";
                            origin.marker = new Object();
                            origin.marker.radius = 4;
                            origin.marker.symbol = "circle";
                            origin.data = [[data[0].x, data[0].y, data[0].z]];
                            if (origin.data[0][2] < minZ) {
                                minZ = origin.data[0][2];
                            }
                            if (origin.data[0][2] > maxZ) {
                                maxZ = origin.data[0][2];
                            }
                            coords[coords.length] = origin;


                        } else {
                            alertTips('尚未计算震源!', 'label-important', 'tipChar', 5000);
                        }
                        axis.minZ = minZ;
                        axis.maxZ = maxZ;
                        create3D(coords, axis);
...
function getCollectTime() {
    var index = $('#vibrationList').next().find(".selected").attr("rel");
    var sensorId = $('#vibrationList').find("option")[index].value;
    var url = apiurl + '/vibration/' + sensorId + '/collecttime?token=' + getCookie('token');
    $.ajax({
        url: url,
        type: "get",
        cache: false,
        success: function (data) {
            ...
            if (data.length != 0) {
                $(data).each(function (i, item) {
                    var dataTime = ChangeDateFormat(item.collectTime);
                    var opt = "<option value='" + item.collectTime + "'>" + dataTime + "</option>";
                    $('#collectTime').append(opt);
                });
            }
            $('#collectTime').selectpicker('refresh');
        }
    });
}
