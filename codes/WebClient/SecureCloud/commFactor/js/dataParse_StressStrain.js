    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                if (itemId == 2) {//判断为杆件应变，2这边只做应力应变判断
                    if (data[i].itemId == item - 1) {
...
//获取采集最大值、最小值、传感器位置
function getMaxMinValue(data,k, index) {
    if (index == null) {
        index = 0;
    }
    var min = "";
    ...
    if (columnsLength == 1) {
        ...
        if (index == 0) {
            if (k == 1) {
                ...
                $('#YmaxValue').html(max + unit);
                $('#YminValue').html(min + unit);
                $('#YsensorLocation').html(sensorLocation);
            }
        }
...
//获得传感器告警
function getSensorWarn(sensorData, sensorList, Startdate, Enddate,k) {
    $('#sensorLink').show();
    //告警等级越小，告警越严重
    ...
    href = "../DataWarningTest.aspx?sensorId=" + sensorList +"&sensorLocation="+sensorData[0].location+ "&startTime=" + Startdate + "&endTime=" + Enddate;
    if (columnsLength == 1) {
        if (k == 1) {
