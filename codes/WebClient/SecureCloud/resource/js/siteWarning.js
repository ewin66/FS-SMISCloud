    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                $('.notification').append('<li><a href="/DataWarningTest.aspx" ><span class="label label-info"><i class="icon-bell"></i></span>' + content + '&nbsp;&nbsp;&nbsp;&nbsp;<span class="time">' + nowDateInterval(GetMilliseconds(data[0].warnings[i].time)) + '</span></a></li>');
                if (i >= 5) { // 列表最多显示6项
                    break;
                }
            }
