...
namespace DataCenter.Accessor
{
    public abstract class DbHelperSQL
    {
        ...
        public static DataTable SelectData(string sql)
        {
            ...
            using (SqlDataAdapter adeapter = new SqlDataAdapter(sql, connection))
            {
                try
                {
                    adeapter.Fill(table);
        ...
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static string ExecuteSqlString(string SQLString)
        {
            DataTable table = new DataTable();
            using (SqlConnection connection = new SqlConnection(connectionString))
            using (SqlDataAdapter adeapter = new SqlDataAdapter(SQLString, connection))
            {
                adeapter.Fill(table);
        ...
        public static void ImportDataIntoDb(DataTable table)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    using (var bulkCopy = new SqlBulkCopy(connection, SqlBulkCopyOptions.CheckConstraints, transaction))
                    {
                        ...
                        foreach (object col in table.Columns)
                        {
