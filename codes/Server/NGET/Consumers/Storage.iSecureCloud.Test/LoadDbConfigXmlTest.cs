    ...
    public class LoadDbConfigXmlTest
    {
        ...
        public void LoadDbConfigXml()
        {
            if (!System.IO.File.Exists(this.path))
                throw new Exception(string.Format("{0} is not exist !", this.path));
        ...
        {
            string connstr = this.loadxml.GetSqlConnectionStrings();
            Assert.AreEqual(
                "Initial Catalog=DW_iSecureCloud_Empty;Data Source=192.168.1.128;User Id=sa;Password=861004", connstr);
        }
        ...
        {
            IList<TableInfo> tablelst = this.loadxml.GeTableInfos();
