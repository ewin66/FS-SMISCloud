...
namespace FS.SMIS_Cloud.NGET.DataValidator
{
    using System;
    ...
    public class DataValidator : IDacTaskResultConsumer
    {
        ...
        public void ProcessResult(List<SensorAcqResult> source)
        {
            ...
            Dictionary<string, ConfigInfo> configInfos = this.GetConfigInfos(source);
            foreach (var sensorResult in source)
            {
                ...
                if (this.SensorTypeFilter.Contains(sensorResult.Sensor.SensorType))
                {
                    ...
                    for (var i = 0; i < sensorResult.Data.ThemeValues.Count; i++)
                    {
                        ...
                        this.RefreshValidatorWindows(validateWindow, configInfo, keyi);
        ...
        private Dictionary<string, ConfigInfo> GetConfigInfos(List<SensorAcqResult> source)
        {
            var configInfos = new Dictionary<string, ConfigInfo>();
            if (source != null && source.Count != 0)
            {
                List<uint> sensorIdList =
                    source.Select(sensorResult => sensorResult.Sensor.SensorID).ToList();
