namespace NGDAC.Test.Tran
{
 ...
 //       public IAdapter myadapter;
        ...
        public void Test()
        {
            ...
            foreach (IAdapter a in this.adapters2)
            {
            }
            foreach (Lazy<IAdapter, IAdapterCap> lazy in this.adapters3)
            {
                Console.WriteLine("Name={0} protocol={1}",lazy.Metadata.Name, lazy.Metadata.Protocol);
            }
            Assert.AreEqual(2, this.adapters3.Count());
            Assert.AreEqual(3, this.adapters1.Count());
            Assert.AreEqual(3, this.adapters2.Count());
            Assert.AreEqual(3, this.adapters4.Count());
        }
    }
