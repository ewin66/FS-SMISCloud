namespace FS.SMIS_Cloud.NGDAC.Tran.Db
{
    ...
    public class TableDataProvider
    {
        ...
        public int GetRemainder()
        {
            if (this.Remainder <= 0)
            {
                string sql = string.Format("select count(*) as CNT from {0} where lastSyncTime is null", this.TableInfo.TableName);
                DataSet ds = this.SqlHelper.Query(sql);
                this.Remainder = Convert.ToInt32(ds.Tables[0].Rows[0]["CNT"]);
            }
        ...
        public int OnDataSynchronized(int[] ids)
        {
            ...
            if (ids.Length >= 0)
            {
                ...
                string sql = string.Format("update {0} set lastSyncTime='{2}' where ID in ({1})", this.TableInfo.TableName, sb, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                updated = this.SqlHelper.ExecuteSql(sql);
