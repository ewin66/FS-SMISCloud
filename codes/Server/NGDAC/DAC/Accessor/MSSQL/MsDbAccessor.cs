...
select DISTINCT A.GROUP_ID ,S.DTU_ID,D.REMOTE_DTU_NUMBER,D.REMOTE_DTU_GRANULARITY
...
SELECT [GROUP_ID]
  ...
  join [dbo].[T_DIM_REMOTE_DTU] D on S.DTU_ID=D.ID and D.ProductDtuId!=3
  order by A.GROUP_ID");
            ...
            groups.AddRange(from DataRow row in ds.Tables[0].Rows
                select new SensorGroup
                {
                    ...
                    GroupId = Convert.ToUInt32(row["GROUP_ID"]),
...
STATUS={0}, FINISHED='{1}', ELAPSED={2}, RESULT_CODE={3}, RESULT_MSG='{4}', RESULT_JSON='{5}' where ID={6}",
        ...
        {
            DataSet ds = this._helper.Query("select * from T_DIM_DAC_ERROR_CODE");
