...
namespace FS.SMIS_Cloud.NGDAC.Task
{
    ...
    public class DACTaskManager :  WorkFinder
    {
        ...
        public static event DACTaskResultListener OnDacTaskFinished;
        ...
        public DACTaskManager(IDtuServer dtuServer, IList<DtuNode> dtus, IList<DACTask> utasks, DtuType type=DtuType.Gprs)
        {
            ...
            if (DtuType.Gprs == type)
                dtuServer.OnConnectStatusChanged += this.OnConnectionStatusChanged;
            ...
            if (utasks != null)
                foreach (DACTask t in utasks)
                {
                    ...
                    _rtasks[t.DtuID] = t;
                    this.ArrangeInstantTask(t.TID, t.DtuID, t.Sensors, null, true);
                }
        }
        ...
        public void AddDtu(DtuNode dtu)
        {
            ...
            this.ScheduleJob(dtu, DateBuilder.NextGivenMinuteDate(null, (int) dtu.DacInterval/60));
        ...
        private void CheckDtuInfo(IDtuConnection c)
        {
            ...
            {
                // 新DTU 上线.
                this.AddDtu(DbAccessorHelper.DbAccessor.QueryDtuNode(c.DtuID));
            }
        }
        ...
        class JobInfo
        {
            ...
            {
                return string.Format("{1}-{0}", this.Dtu.DtuCode, this.Type);
            }
        }
        ...
        DACTask CreateInstantDACTask(string tid, uint dtuId, IList<uint> sensors, TaskResultConsumer consumer, bool activeFromDB)
        {
            ...
            if (activeFromDB)
            {
                if (this.HasUnfinishedTask(dtuId, out task))
                    return task;
            }
            else
            {
        ...
        private DACWorker FindWorker(string dtuCode)
        {
            if (_dtuCodeMap.ContainsKey(dtuCode)) {
                ...
                return this.FindWorker(d.DtuId);
            }
            return null;
        }
        ...
        private DacTaskContext CreateContext(uint dtuId)
        {
            ...
            if (_ctx.Node.NetworkType == NetworkType.hclocal)
                _ctx.DtuConnection = this._fileServer.GetConnection(_ctx.Node);
            else
                _ctx.DtuConnection = this._dtuServer.GetConnection(_ctx.Node);
            return _ctx;
        }
        ...
        {
            if (this._senconfChangedServer == null)
            {
                this._senconfChangedServer = new SensorConfigUpdateServer();
                this._senconfChangedServer.GetDtuNodeListener += this.senconfChangedServer_GetDtuNodeListener;
                this._senconfChangedServer.StartWork();
            }
            this._senconfChangedServer.TryAddNewSensorOperation(operation);
        }
        ...
        public bool DtuConfigChanged(ChangedStatus cmd, DtuNode dtu,string dtucode=null)
        {
            switch (cmd)
            {
                case ChangedStatus.Add:
                    return dtu.NetworkType == NetworkType.gprs ? this._dtuServer.Disconnect(dtu.DtuCode) : this._fileServer.Disconnect(dtu);
                case ChangedStatus.Modify:
                   return this.ModifyDtu(dtu, dtucode);
                case ChangedStatus.Delete:
                   return this.RemoveDtu(dtu);
            }
            return false;
        }
        ...
        private bool ModifyDtu(DtuNode dtu,string dtucode)
        {
            ...
            return this.TryRemoveJob(dtu.DtuCode);
        }
        ...
        {
            var jk = new JobKey(this.GetDailyJobKey(dtuCode), JobGroup);
        ...
        public bool ModifyJobTime(DtuNode dtu)
        {
            if (_schedule == null)
                return false;
            string triggerKeystr = this.GetDailyTriggerKey(dtu.DtuId);
            ITrigger trigger = _schedule.GetTrigger(new TriggerKey(triggerKeystr, JobGroup));
            if (trigger != null)
            {
                _schedule.PauseJob(new JobKey(this.GetDailyJobKey(dtu.DtuCode), JobGroup));
                this.TryRemoveJob(dtu.DtuCode);
