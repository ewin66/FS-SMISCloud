...
namespace FS.SMIS_Cloud.NGDAC.DAC.CxxAdapter
{
    ...
    public class Gps_HC_SensorAdapter : GpsBaseAdapter
    {
        public Gps_HC_SensorAdapter()
        {
            ...
            this._idxDY = 11;
            this._idxDH = 12;
            this._log = LogManager.GetLogger("GpsHcAdapter");
            this._dtformat = new DateTimeFormatInfo { ShortDatePattern = "yyyy/MM/dd" };
        }
#if READ_GPS_FIRST
            ...
            using (var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                using (var reader = new StreamReader(fs))
                {
                    ...
                    while ((data = reader.ReadLine()) != null)
                    {
                        if (string.IsNullOrEmpty(data)) continue;
                        string[] d = data.Split(',');
                        string fId = d[this._idxModule];
                        DateTime fTime = Convert.ToDateTime(d[this._idxTime], this._dtformat);
