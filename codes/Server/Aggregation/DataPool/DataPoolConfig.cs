// // --------------------------------------------------------------------------------------------
// // <copyright file="ProcessConfig.cs" company="江苏飞尚安全监测咨询有限公司">
...
namespace Agg.DataPool
{
    ...
    using log4net;
    public abstract class DataPoolConfig
    {
        private static ILog Log = LogManager.GetLogger("DataPoolConfig");
        public static Dictionary<string, Type> FromXML(string fileName)
        {
            Dictionary<string, Type> ret = new Dictionary<string, Type>();
            if (!System.IO.File.Exists(fileName))
            {
                Log.Warn("DataPool.xml not found");   
                return ret;
            }
            ...
            try
            {
                ...
                var nodes = doc.Root.Element("DataPools").Elements();
            ...
            catch (Exception e)
            {
                ...
                Log.Warn("DataPool.xml parse error", e);
            }
            return ret;
        }
    }
