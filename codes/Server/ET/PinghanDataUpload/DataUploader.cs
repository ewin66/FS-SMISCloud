...
namespace PinghanDataUpload
{
    ...
    public class DataUploader : IDACTaskResultConsumer
    {
        ...
        public DataUploader()
        {
            Config = ReloadBasicConfig();
            ConsoleCtrlManager.Instance.RegCmd("pud", UploadHandler);
        }
...
WHERE SENSOR_ID = {0} and (ACQUISITION_DATETIME >= '{1:yyyy-MM-dd HH:mm:ss}' and ACQUISITION_DATETIME <='{2:yyyy-MM-dd HH:mm:ss}')",
                ...
                try
                {
                    ...
                    {
                        var request = CreateRequest(data, item);
                        Console.WriteLine("\tupload item: {0}", request);
                        var result = UploadRequest(request);
        ...
        {
            _log.DebugFormat("upload item: {0}", requestUrl);
            try
            {
                ...
                //DEBUG return;
                if (Config.EnableDebug == (int) Switch.On) return load;
                ...
                {
                    _log.Error("Failed to upload item.");
                    return "Failed.";
                }
            ...
            {
                _log.Error("Failed to upload item", e);
                return e.Message;
            }
        }
                ...
                if (sensorResult.IsOK && sensorResult.Data != null)
                {
                    if (items.ContainsKey(sensorResult.Sensor.SensorID)) //是需要上传的数据
                    {
                        try
                        {
                            ...
                            var request = CreateRequest(sensorResult, item, source.Finished);
                            //不上报异常
                            if (item.FHDZ != "" && Config.EnableException == (int) Switch.Off) return;
                            var result = UploadRequest(request);
                            _log.DebugFormat("\tresult: {0}", result);
                        }
                        catch (Exception ex)
                        {
                            _log.ErrorFormat("\tresult: {0}", ex.Message);
                        }
                    }
                }
            }
