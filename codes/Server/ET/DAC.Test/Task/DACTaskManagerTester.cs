...
namespace DAC.Test.Task
{
    ...
    public class DACTaskManagerTester
    {
        ...
        public void TestRemoteDtuRead()
        {
            ...
            Log.DebugFormat("Connect to db: {0}", sqlconn);            
        ...
        public void TestMsSqlInstantTask()
        {
            ...
            DbAccessorHelper.Init(new MsDbAccessor(mssqlConnStr));
            // Thread.Sleep(8000);
            string tid = new Guid().ToString();
            List<uint> sensors = new List<uint> { (uint) 17 };
            DACTask ut = new DACTask(tid, 1, sensors, TaskType.INSTANT, OnTaskFinished); //
