...
namespace FS.SMIS_Cloud.DAC.Util
{
    public class ValueHelper
    {
        ...
        {
            len = len == 0 ? da.Length : Math.Min(len, da.Length);
        ...
        public static string CreateJsonResultStr(uint senId, string datastr)
        {
            return string.Format("{0}\"sensorId\":{1},\"data\":\" {2} \"{3}", '{', senId, datastr, '}');
        }
    }
}
