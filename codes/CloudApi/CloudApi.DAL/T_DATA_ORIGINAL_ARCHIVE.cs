//------------------------------------------------------------------------------
// <auto-generated>
//    此代码是根据模板生成的。
//
//    手动更改此文件可能会导致应用程序中发生异常行为。
//    如果重新生成代码，则将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace FreeSun.FS_SMISCloud.Server.CloudApi.DAL
{
    using System;
    using System.Collections.Generic;
    
    public partial class T_DATA_ORIGINAL_ARCHIVE
    {
        public int Id { get; set; }
        public int SensorId { get; set; }
        public Nullable<decimal> Value1 { get; set; }
        public Nullable<decimal> FilterValue1 { get; set; }
        public Nullable<decimal> Value2 { get; set; }
        public Nullable<decimal> FilterValue2 { get; set; }
        public Nullable<decimal> Value3 { get; set; }
        public Nullable<decimal> FilterValue3 { get; set; }
        public Nullable<decimal> Value4 { get; set; }
        public Nullable<decimal> FilterValue4 { get; set; }
        public bool IsFilter { get; set; }
        public System.DateTime CollectTime { get; set; }
    }
}
